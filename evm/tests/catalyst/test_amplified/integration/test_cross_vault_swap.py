...
def test_cross_vault_swap(
    ...
    swap_amount = int(source_token.balanceOf(vault_1) * swap_percentage)
    ...
    try:
        ...
        pytest.skip()  # TODO For certain vault configs, there is no output for the specified swap amount (swap too large) => remove this workaround + be specific on the test configs to 'skip'
    ...
    if vault_2.getUnitCapacity() < vault_2.calcReceiveAsset(
        vault_2._tokenIndexing(0), tx.events["SendAsset"]["Units"]
    ) * vault_2._weight(vault_2._tokenIndexing(0)):
    ...
    assert purchased_tokens <= y + 5 or purchased_tokens <= int(
        y * 1.000001
...
# TODO add test that empties the vault
...
def test_cross_vault_swap_min_out(
    ...
    # Check that the transaction ack on fail.
