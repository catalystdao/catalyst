...
def pool_set_fees(request, pool, gov):

    pool_fee = request.param[0]
    governance_fee = request.param[1]

    pool.setPoolFee(int(pool_fee * 10**18), sender=gov)
    pool.setGovernanceFee(int(governance_fee * 10**18), sender=gov)
...
@pytest.fixture(scope="module", params=FEES, ids=pool_fee_ids_fn)
def group_set_fees(request, group_pools, gov):
