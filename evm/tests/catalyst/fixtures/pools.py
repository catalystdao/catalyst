...
from utils.pool_utils import compute_balance_0, compute_invariant

from utils.pool_utils import compute_expected_max_unit_inflow
...
def deploy_pool(project, accounts, swap_factory, volatile_swap_pool_template, amplified_swap_pool_template,  cross_chain_interface, swap_pool_type, gov):
    def _deploy_pool(
        ...
        if template_address == volatile_swap_pool_template.address:
            return project.CatalystSwapPoolVolatile.at(tx.return_value)
        else:
            return project.CatalystSwapPoolAmplified.at(tx.return_value)
