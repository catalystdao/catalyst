...
from utils.swap_utils import run_local_swap
...
@given(swap_amount=strategy("uint256", max_value=depositValues[0]))
def test_local_swap(
    swappool1_info,
    swapper,
    gov,
    ...
    for token, init_balance in zip(tokens, init_balances):
        assert_relative_error(token.balanceOf(sp), init_balance, -1e-4, 1e-6)
