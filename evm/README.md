...
Running `foundry compile` or any similar function which also compiles the contracts like `foundry test` will fetch the other dependencies as submodules.
...
For more about tests, read the readme within `./test`.
...
Let create a function which executes a local swap. That is a swap which happens atomically on a single chain to and from the same vault.

To ensure that we have a clean baseline, we will make another account which will do the swap. This way, we can check its balances without taking into account that we deployed the tokens and have an excess.

For our example, we will be using the token at index 0 as from and token at index 1 as to. We will be swapping 100 tokens.
...
We are now ready to execute a localswap.
...
Notice we set the minout to 0. You can play around with setting it to 50, 80, or 100? Remember to add decimals: 50 tokens is 50 * 10**18.
The last line checks that the vault didn't lie about how much we purchased.

Lets run the test: `forge test --match-test test_localswap -vvvv`.
