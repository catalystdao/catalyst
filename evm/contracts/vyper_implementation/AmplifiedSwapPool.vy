...
@notice
    ...
    pricing curve: W/(w^k) · (1 - k) where W is an asset specific
    weight and w is the pool balance.
    This is an amplified price curve (lim_{k -> 0} P_k(w) = 1)
...
# To distribute the fees, the pool needs to keep track of its relative
# ownership of units in the system. This is only needed for amplified pools.
unitTracker: int128
...
# Amplification
amp: public(uint256)  # !MUST BE THE SAME FOR EVERY CONNECTED POOL!
...
ampUnitCONSTANT: uint256
ampLiquidityUnitCONSTANT: uint256  # TODO
...
def setup(chaininterface : address, init_assets : address[NUMASSETS], weights : uint256[NUMASSETS], amp : uint256, name : String[16], symbol: String[8], setupMaster : address):
    """
    @param amp Amplification in X64, Cannot be set to 0. 
    @notice Setup a pool. 
    @dev
        amp cannot be set to 0. 
    ...
    for asset in init_assets:
        ...
        # The maximum unit flow is \sum ampUnitCONSTANT · balance^{1-amp} 
        #   = ampUnitCONSTANT · \sum balance^{1-amp}, so we multiply
        # after the sum to save math operations.
        max_unit_inflow += weights[it] * self._fpowX64(shift(balanceOfSelf, 64), ONE - amp)
    ...
    self.ampUnitCONSTANT = ONE - self._invp2X64(ONE - amp)
    self.max_unit_inflow = self.pMULX64(self.ampUnitCONSTANT, max_unit_inflow)
...
@view
@internal
def getModifyUnitCapacity(oldBalance: uint256, newBalance: uint256, asset: address) -> uint256:
    """
    @notice 
        The maximum unit flow changes depending on the current balance
        for amplified pools. The change is 
            (1-2^(k-1)) · W · b^(1-k) - (1-2^(k-1)) · W · a^(1-k)
        where b is the balance after the swap and a is the balance
        before the swap.
    @dev
        Always returns uint256. If oldBalance > newBalance,
    ...
    @param 
        Returns the absolute change in unit capacity
        Returns without the ampConstant.
    """
    ...
    if oldBalance < newBalance:
        ...
        return self.weight[asset] * (self._fpowX64(shift(newBalance, 64), oneMinusAmp) - self._fpowX64(shift(oldBalance, 64), oneMinusAmp))
    ...
    return self.weight[asset] * (self._fpowX64(shift(oldBalance, 64), oneMinusAmp) - self._fpowX64(shift(newBalance, 64), oneMinusAmp))
...
@view
@internal
def _compute_integral(_in : uint256, _A : uint256, _W : uint256, _amp : uint256) -> uint256:
    """
    @notice
        Computes the integral \int_{A}^{A+in} W/w^k · (1-k) dw
            = W · ( (A + _in)^(1-k) - A^(1-k) )
    ...
    @param _amp The amplification.
    @return Group specific units in X64 (units are **always** X64).
    """
    oneMinusAmp : uint256 = ONE - _amp  # Minor gas saving :)
    return _W * (self._fpowX64(shift(_A + _in, 64), oneMinusAmp) - self._fpowX64(shift(_A, 64), oneMinusAmp))


@view
@internal
def _solve_integral(_U : uint256, _B : uint256, _W : uint256, _amp : uint256) -> uint256:
    """
    @notice
        Solves the equation U = \int_{A-_out}^{A} W/w^k · (1-k) dw for _out
            = B · 
            ( 
                1 - (
    ...
    W_BTimesBtoOneMinusAmp : uint256 = _W * self._fpowX64(shift(_B, 64), oneMinusAmp) 
    return shift(_B * (ONE - self._invfpowX64(self.bigdiv64(W_BTimesBtoOneMinusAmp, W_BTimesBtoOneMinusAmp - _U), self.bigdiv64(ONE, oneMinusAmp))), -64)


@view
@internal
def _complete_integral(_in : uint256, _A : uint256, _B : uint256, _W_A : uint256, _W_B : uint256, _amp : uint256) -> uint256:
    """
    @notice
        Solves the equation 
        ...
        Alternatively, the integral can be computed through:
            _solve_integral(_compute_integral(_in, _A, _W_A, _amp), _B, _W_B, _amp).
    ...
    """
    oneMinusAmp : uint256 = ONE - _amp  # Minor gas saving :)
    W_BTimesBtoOneMinusAmp : uint256 = _W_B * self._fpowX64(shift(_B, 64), oneMinusAmp)
    return shift(_B  * (ONE - self._invfpowX64(
        self.bigdiv64(W_BTimesBtoOneMinusAmp, W_BTimesBtoOneMinusAmp - _W_A * (self._fpowX64(shift(_A + _in, 64), oneMinusAmp) - self._fpowX64(shift(_A, 64), oneMinusAmp)))
    , self.bigdiv64(ONE, oneMinusAmp))), -64)
    

@view
@internal
def _dry_swap_to_unit(_from : address, _amount : uint256) -> uint256:
    """
    @notice
    ...
    assert A + _amount < 2**(256-64) - 1    # dev: Potential exploitable
                                            # if the attacker can abuse
                                            # some kind of flash-mint.
    W: uint256 = self.weight[_from]

    return self._compute_integral(_amount, A, W, self.amp)


@view
@external
def dry_swap_to_unit(_from : address, _amount : uint256) -> uint256:
    """
    @notice
        Computes the return of a SwapToUnits, without executing one.
    @dev
        Unlike a non-amplified pool, there is not simple swap curve
        approximation for amplified pools.
    @param _from The address of the token to sell.
    @param _amount The amount of _from token to sell.
    @return Group specific units in X64 (units are **always** X64).
    """
    return self._dry_swap_to_unit(_from, _amount)


@view
@internal
def _dry_swap_from_unit(_to : address, U : uint256) -> uint256:
    """
    @notice
    ...
    assert B < 2**(256-64) - 1  # dev: Potential exploitable
                                # if the attacker can abuse
                                # some kind of flash-mint.

    return self._solve_integral(U, B, W, self.amp)


@view
@external
def dry_swap_from_unit(_to : address, U : uint256) -> uint256:
    """
    @notice
        Computes the output of a SwapFromUnits, without executing one.
    @dev
    ...
    """
    return self._dry_swap_from_unit(_to, U)


@view
@internal
def _dry_swap_both(_from : address, _to : address, _in: uint256) -> uint256:
    """
    @notice
    ...
    assert B < 2**(256-64) - 1  # dev: Potential exploitable
                                # if the attacker can abuse
                                # some kind of flash-mint.
    assert A + _in < 2**(256-64) - 1    # dev: Potential exploitable
                                        # if the attacker can abuse
                                        # some kind of flash-mint.
    
    return self._complete_integral(
        _in, A, B, W_A, W_B, self.amp
    )


@view
@external
def dry_swap_both(_from : address, _to : address, _amount: uint256) -> uint256:
    """
    @notice
        Computes the return of a SwapToAndFromUnits, without executing one.
    @param _from The address of the token to sell.
    ...
    @return Output denominated in _to token.
    """
    return self._dry_swap_both(_from, _to, _amount)
...
#     # if "At * poolTokens" > 2**256 - 1, then the pool is semi-frozen.
#     # Since At <= 2**256 - 1, then it should always be possible to
#     # provide poolTokens = 1. Thus if someone slowly goes through
#     # poolToken = 1, poolToken = 1 + ..., poolToken = 1 + ... + ...
#     # then they will eventually unfreeze the pool. This requires a
#     # stupid amount of tokens and is a complete waste. The simplest way
#     # to get this number of tokens, would be flash-mints. However, they
#     # would have to return those (thus withdraw) at the end of the tx.
#     # Thus it is not a real attack surface.
...
def depositAll(_baseAmount : uint256):
    ...
    for asset_num in range(NUMASSETS):
        ...
        # Right here, we should check that the deposit doesn't soft-freeze
        # the pool. However, doing this deposit would be incredibly pointless.
        # As the withdrawal equation does not have the same vulnerability as
        # the swap functions.
        # As such, people would always be able to withdraw.
        # Here is the line: assert ERC20(asset).balanceOf(self) + base < 2**(256-64) - 1 
...
@external
@nonreentrant('lock')
def localswap(_fromAsset : address, _toAsset : address, _amount : uint256, minOut: uint256) -> uint256:
    """
    @notice
    ...
    out: uint256 = self._dry_swap_both(_fromAsset, _toAsset, _amount - fee)
    ...
    unitConstant : uint256 = self.ampUnitCONSTANT
...
@external
@nonreentrant('lock')
def swapToUnits(_chain : uint256, _targetPool : bytes32, _fromAsset : address, _toAsset : uint256, _who : bytes32, _amount : uint256, _minOut : uint256, fallbackUser : address = msg.sender) -> uint256:
    """
    @notice
    ...
    U: uint256 = self._dry_swap_to_unit(_fromAsset, _amount - fee)
    ...
    IBCInterface(self.chaininterface).crossChainSwap(_chain, False, _targetPool, _who, U, _toAsset, _minOut, sourceSwapId)
    ...
    self.unitTracker += convert(U, int128)  # Todo: security

    # Compute the change in max_unit_inflow.
    _fromBalance: uint256 = ERC20(_fromAsset).balanceOf(self)
    self.max_unit_inflow += self.pMULX64(self.ampUnitCONSTANT, self.getModifyUnitCapacity(_fromBalance - _amount, _fromBalance, _fromAsset))
    ...
    log SwapToUnits(_who, _fromAsset, _toAsset, _chain, _amount, U, fee)
...
@external
@nonreentrant('lock')
def swapFromUnits(_toAsset : uint256, _who : address, _U : uint256) -> uint256:
    """
    @notice
    ...
    # Calculate the swap return value.
    purchasedTokens: uint256 = self._dry_swap_from_unit(toAsset, _U)
    # Send the return value to the user.
    self.safeTransfer(toAsset, _who, purchasedTokens)

    # Track units for fee distribution.
    self.unitTracker -= convert(_U, int128)

    # Compute the change in max_unit_inflow.
    _toBalance: uint256 = ERC20(toAsset).balanceOf(self)
    self.max_unit_inflow -= self.pMULX64(self.ampUnitCONSTANT, self.getModifyUnitCapacity(_toBalance + purchasedTokens, _toBalance, toAsset))
...
@view
@internal
def _compute_liquidity(_in : uint256, _A : uint256, _W : uint256, _amp : uint256) -> uint256:
    """
    @notice
        Computes the integral \int_{A-out}^{A} W/w^k · (1-k) dw
            = - W · ( A^(1-k) - (A - _out)^(1-k) )
        For optimisations, U is always assumed to be negative for liquidity units.
        Thus the function computes:  W · ( A^(1-k) - (A - _out)^(1-k) )
    ...
    """
    oneMinusAmp : uint256 = ONE - _amp  # Minor gas saving :)
    return _W * (self._fpowX64(shift(_A, 64), oneMinusAmp) - self._fpowX64(shift(_A - _in, 64), oneMinusAmp))
    


@view
@internal
def _solve_liquidity(_U : uint256, _B : uint256, _WSUM : uint256, _amp : uint256) -> uint256:
    """
    @notice
        Solves the equation U = \int_{A}^{A+_out} W/w^k · (1-k) dw for _out
            = B · 
            ( 
                (
                    (W_B · B^(1-k) - U) / (W_B · B^(1-k))
            ...
            Since _U is negative units, the
            function actually computes: B · 
            ( 
                (
                    (W_B · B^(1-k) + U) / (W_B · B^(1-k))
                )^(1/(1-k)) - 1
            )
        Note that the specific weight divides into the equation such that the is substrationally simplified.
    ...
    """
    return  shift(_B * (self._fpowX64(self.bigdiv64(_WSUM + _U, _WSUM), self.bigdiv64(ONE, ONE - self.amp)) - ONE), -64)


@view
@internal
def _dry_liquidity_to_unit(_from : address, _amountX64 : uint256) -> uint256:
    """
    @notice
    ...
    W: uint256 = self.weight[_from]
    assert A < 2**(256-64) -1   # dev: Potential exploitable
                                # if the attacker can abuse
                                # some kind of flash-mint.
    return self._compute_liquidity(_amountX64, A, W, self.amp)
...
def _dry_liquidity_from_unit(_to : address, U : uint256, _WSUM : uint256) -> uint256:
    ...
    return self._solve_liquidity(U, B, _WSUM, self.amp)
...
@internal
def _distributeFees() -> uint256:
    WSUM : uint256 = 0
    tokens: address[3] = empty(address[3])
    amp : uint256 = ONE - self.amp
    calculatedOutstandingUnits : int128 = 0
    for i in range(3):
        token : address = self.tokenIndexing[i]
        if token == ZERO_ADDRESS:
            break
        b0 : uint256 = self.balance0[token]
        bt : uint256 = ERC20(token).balanceOf(self)
        w : uint256 = self.weight[token]
        if bt > b0:
            calculatedOutstandingUnits += convert(self._compute_liquidity(bt-b0, b0, w, amp), int128)
        else:  # b0 > bt
            calculatedOutstandingUnits -= convert(self._compute_liquidity(b0-bt, bt, w, amp), int128)
        tokens[i] = token
        WSUM += w * self._fpowX64(b0, ONE - amp)
    
    calculatedOutstandingUnits = self.unitTracker - calculatedOutstandingUnits
    # Because of the balance0 constraint of wrapped pool tokens,
    # only the first balance0 change needs to be computed.
    token0B0 : uint256 = self._dry_liquidity_from_unit(tokens[0], convert(calculatedOutstandingUnits, uint256), WSUM)

    ts : uint256 = self.totalSupply
    # Token0B0 * _baseAmount = self.balance0
    poolTokens : uint256 = (token0B0*ts)/self.balance0[tokens[0]]
    self.balance0[tokens[0]] += token0B0


    for i in range(1, 3):
        if tokens[i] == ZERO_ADDRESS:
            break
        self.balance0[tokens[i]] += (self.balance0[tokens[i]] * poolTokens)
    
    return poolTokens


@external
def distributeFees() -> uint256:
    return self._distributeFees()
...
def outLiquidity(_chain : uint256, _targetPool : bytes32, _who : bytes32, _baseAmount : uint256) -> uint256:
    ...
    log SwapToUnits(_who, self, 2**8-1, _chain, _baseAmount, U, 0)
...
def inLiquidity(_who : address,  _U : uint256) -> uint256:
    ...
    WSUM : uint256 = 0  # is X64.
    tokens: address[NUMASSETS] = empty(address[NUMASSETS])
    oneMinusAmp : uint256 = ONE - self.amp
    for i in range(NUMASSETS):
        token : address = self.tokenIndexing[i]
        if token == ZERO_ADDRESS:
            break
        tokens[i] = token
        WSUM += self.weight[token] * self._fpowX64(shift(self.balance0[token], 64), oneMinusAmp)
